...
async def _transcribe_chunk_openai(client: AzureOpenAI, audio_chunk_data: bytes, chunk_index: int) -> str:
    ...
    try:
        ...
        logger.info(f"Transcribing chunk {chunk_index} with OpenAI (model: {settings.MODEL}, from memory)...")
        ...
        logger.error(f"OpenAI API Error for chunk {chunk_index}: Status={e.code}, Message={e.message}")
...
async def _perform_simple_transcription(client: AzureOpenAI, audio_path: str) -> str:
    ...
    try:
        ...
        logger.error(f"OpenAI API Error (standard): Status={e.code}, Message={e.message}")
        raise OpenAIError(f"OpenAI API Error: {e.message}", status_code=e.code) from e
    except Exception as e:
...
async def _perform_chunked_transcription(client: AzureOpenAI, audio_path: str, file_size_bytes: int) -> str:
...
async def process_and_transcribe(converted_audio_path: str) -> str:
    ...
    try:
        client = AzureOpenAI(api_key=settings.AZURE_OPENAI_API_KEY, azure_endpoint=settings.AZURE_OPENAI_ENDPOINT, api_version=settings.AZURE_OPENAI_API_VERSION)
    except Exception as e:
